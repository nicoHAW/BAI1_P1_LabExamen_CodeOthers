...
import static _untouchable_.supportC1x01.Configuration.mainPartOfTestResultDataBaseFileName;
import static _untouchable_.supportC1x01.Configuration.mainPartOfTestResultDataBasePath;
...
public class TestResultDataBaseManager {
    ...
    synchronized public void enterLocally(
        ...
        final TL testLevel = testTopic.level();
        final TC testCategory = testTopic.category();
        final String testName = testResult.testMethodName();
        final int testWeight = testResult.weight();
        //
        /*
        ...
        switch( testLevel ){
            case I:
                {//scope
                    ...
                    if( ! matcher.matches() ){
                        Herald.proclaimError( String.format( "[%s]:  tol_0i_ <-> %s\n",  testLevel, testName ));
            ...
            case A:
                {//scope
                    ...
                    final Matcher matcher = pattern.matcher( testName );
                    if( ! matcher.matches() ){
                        Herald.proclaimError( String.format( "[%s]:  tol_1e_ <-> %s\n",  testLevel, testName ));
                        throw new IllegalStateException( "Internal coding error - did not expect to end up \"here\" - call Michael Schaefers" );
                    }//if
                }//scope
                break;
            case B:
                {//scope
                    ...
                    final Matcher matcher = pattern.matcher( testName );
                    if( ! matcher.matches() ){
                        Herald.proclaimError( String.format( "[%s]:  tol_2b_ <-> %s\n",  testLevel, testName ));
                        throw new IllegalStateException( "Internal coding error - did not expect to end up \"here\" - call Michael Schaefers" );
                    }//if
                }//scope
                break;
            case C:
                {//scope
                    ...
                    final Matcher matcher = pattern.matcher( testName );
                    if( ! matcher.matches() ){
                        Herald.proclaimError( String.format( "[%s]:  tol_3n_ <-> %s\n",  testLevel, testName ));
                        throw new IllegalStateException( "Internal coding error - did not expect to end up \"here\" - call Michael Schaefers" );
                    }//if
                }//scope
                break;
            case D:
                {//scope
                    ...
                    final Matcher matcher = pattern.matcher( testName );
                    if( ! matcher.matches() ){
                        Herald.proclaimError( String.format( "[%s]:  tol_4s_ <-> %s\n",  testLevel, testName ));
                        throw new IllegalStateException( "Internal coding error - did not expect to end up \"here\" - call Michael Schaefers" );
                    }//if
                }//scope
                break;
            default:
                {//scope
                    Herald.proclaimError( String.format( "[%s]:  Did not expect to end up \"here\": %s\n",  testLevel, testName ));
    ...
    synchronized public void updateSynchronizedExerciseRelatedResults(){
        ...
        for( TestTopic testTopic : currentTestResults.keySet() ){
            final TE testExercise = testTopic.exercise();
        ...
        try(
            ...
            if( null != allTestResults ){
                ...
                Herald.proclaimMessage( "note: NO results at all\n" );
            }//if
    ...
    public TestResultDataBaseManager(                                           // package scope on purpose
        ...
        if( ! (
            ...
            Herald.proclaimError( errorMessage );
